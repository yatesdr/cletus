...
func (c *ChatView) AddMessage(role, content string) {
	c.finishStream()
	if role == "user" {
		c.pinnedToBottom = true // always scroll to show new user input
	}
...
// ScrollUp scrolls the chat view up and disables auto-scroll.
func (c *ChatView) ScrollUp(lines int) {
	c.pinnedToBottom = false
...
// ScrollDown scrolls the chat view down and re-enables auto-scroll.
func (c *ChatView) ScrollDown(lines int) {
	c.pinnedToBottom = true
	row, col := c.view.GetScrollOffset()
	c.view.ScrollTo(row+lines, col)
}
