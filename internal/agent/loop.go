	...
	for {
		...
		err := l.client.Stream(ctx, req, func(event api.StreamEvent) {
			...
			case "content_block_start":
				...
				handler(Event{Type: "content_block_start", ContentBlock: event.ContentBlock})
			case "content_block_stop":
				currentBlockType = ""
				handler(Event{Type: "content_block_stop"})
			case "content_block_delta":
				if event.Content != "" {
					if currentBlockType == "text" || currentBlockType == "" {
						assistantContent.WriteString(event.Content)
					}
					handler(Event{Type: "content_block_delta", Content: event.Content, ContentBlock: event.ContentBlock})
				}
			case "tool_use":
